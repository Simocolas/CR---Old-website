...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    await sendEmailsWithResend(emailData);
...
async function sendEmailsWithResend(emailData) {
  const resend = new Resend(process.env.RESEND_API_KEY);
  const businessEmail = process.env.BUSINESS_EMAIL || 'info@calgarycityroofing.com';
  ...
  const businessEmailContent = {
    from: fromEmail,
    to: businessEmail,
    subject: `🚨: Contact Form Submission from ${emailData.name}`,
    html: generateBusinessEmailHTML(emailData)
  };
  ...
  // Send both emails
  await Promise.all([
    resend.emails.send(businessEmailContent),
    resend.emails.send(customerEmailContent)
  ]);

  console.log(`✅ Emails sent successfully via Resend for ${emailData.name}`);
}
